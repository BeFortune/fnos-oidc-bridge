...
5. **反向代理**:把 `https://<你的统一域名>/oidc/` 反代到 `http://127.0.0.1:4223`，**保持 `/oidc` 前缀不变**，并且不要给这个 location 套 `auth_request`、访问码或飞牛统一网关鉴权；OIDC 的 token/userinfo 是下游服务端调用，没有飞牛浏览器 Cookie。
